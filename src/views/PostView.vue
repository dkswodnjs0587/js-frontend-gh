...
<template>
  <div class="subpage narrow page-section">
    ...
    <article class="post-detail">
      ...
      <div class="post-like-area"><button :class="['post-like-button', { liked }]" :disabled="liking" @click="addLike"><span>{{ liked ? '♥' : '♡' }}</span>{{ liking ? '반영 중…' : liked ? '좋아요 취소' : '좋아요' }} <b>{{ post.likeCount ?? 0 }}</b></button><small v-if="likeError" class="like-error">{{ likeError }}</small></div>
    </article>
