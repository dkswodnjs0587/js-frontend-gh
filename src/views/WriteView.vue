...
function previousWriteTutorial() { if (tutorialStep.value > 0) showTutorialStep(tutorialStep.value - 1); else router.push('/community?tutorial=interaction') }
function handleTutorialKeydown(event) { if (!tutorialActive.value || event.repeat) return; if (event.key === 'Escape') { event.preventDefault(); tutorialActive.value = false; return } if (event.target instanceof HTMLElement && event.target.matches('input, textarea, select')) return; if (event.code === 'Space') { event.preventDefault(); nextWriteTutorial(); return } if (event.key === 'Backspace') { event.preventDefault(); previousWriteTutorial() } }
